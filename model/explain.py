...
def transformed_feature_owners(pipeline: Pipeline) -> list[str]:
    ...
    # Built from the fitted categories: "cat__checkin_acc_A11" cannot be split reliably.
    onehot = (
...
def build_explainer(pipeline: Pipeline, background: np.ndarray) -> shap.LinearExplainer:
    """Builds the explainer over the whole background, so explanations reproduce."""
    # SHAP subsamples by default, which would explain one application two ways.
